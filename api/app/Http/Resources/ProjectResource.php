...
class ProjectResource extends JsonResource
{
    public function toArray(Request $request): array
    {
        ...
        return [
            ...
            'billing_mode' => $this->billing_mode ?? ($this->is_billable ? 'hourly' : 'none'),
            'budget_amount' => $this->budget_amount,
            'billed_amount' => (float) ($this->billed_amount ?? 0),
            'fixed_remaining' => $this->when($this->billing_mode === 'fixed' && $this->budget_amount !== null, fn () => round((float) $this->budget_amount - (float) ($this->billed_amount ?? 0), 2)),
